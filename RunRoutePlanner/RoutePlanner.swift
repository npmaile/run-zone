...
class RoutePlanner: ObservableObject {
    ...
    // Alternative: Create a figure-8 route for variety
    private func createFigureEightRoute(
        center: CLLocationCoordinate2D,
        targetDistance: Double,
        segments: Int
    ) -> [CLLocationCoordinate2D] {
        let radius = targetDistance / (4 * .pi) // Figure-8 is roughly twice the circumference
        ...
        for i in 0...segments {
            let t = 2 * .pi * Double(i) / Double(segments)

            // Parametric equation for figure-8 (lemniscate)
            let scale = radiusInDegrees * sin(t)
            let lat = center.latitude + scale * cos(t)
            let lon = center.longitude + scale * sin(t) * cos(t) / cos(center.latitude * .pi / 180)
    ...
    // Create a rectangular route for urban running
    private func createRectangularRoute(
        center: CLLocationCoordinate2D,
        targetDistance: Double
    ) -> [CLLocationCoordinate2D] {
        let metersPerDegree = 111320.0
        ...
        // Let's make it roughly 2:1 ratio
        let width = targetDistance / 6
        let length = targetDistance / 3

        let widthInDegrees = width / metersPerDegree
        let lengthInDegrees = length / metersPerDegree

        return [
            center,
            CLLocationCoordinate2D(
                latitude: center.latitude + lengthInDegrees,
                longitude: center.longitude
            ),
            CLLocationCoordinate2D(
                latitude: center.latitude + lengthInDegrees,
                longitude: center.longitude + widthInDegrees / cos(center.latitude * .pi / 180)
            ),
            CLLocationCoordinate2D(
